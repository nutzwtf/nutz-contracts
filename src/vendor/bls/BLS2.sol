...
// Vendored from randa-mu/bls-solidity, src/libraries/BLS2.sol, tag v0.3.0,
...
// No functional edits. The only additions are the `// triage:` comment lines that answer forge lint and Aderyn inline.
pragma solidity ^0.8;
...
library BLS2 {
    ...
    function hashToPoint(bytes memory dst, bytes memory message) internal view returns (PointG1 memory out) {
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            // triage: two fixed iterations; a failed precompile call must abort.
    ...
    function expandMsg(bytes memory DST, bytes memory message, uint8 n_bytes) internal pure returns (bytes memory) {
        ...
        // triage: RFC 9380 section 5.3.1 layout; the DST is length-suffixed and domainLen <= 255 is checked above.
        // forge-lint: disable-next-item(encode-packed-collision, unsafe-typecast)
        ...
        // triage: domainLen <= 255 is checked above.
        ...
        for (uint256 i = 1; i < ell; i++) {
            // triage: i < ell <= 8 because n_bytes is a uint8; domainLen <= 255 is checked above.
            // forge-lint: disable-next-line(unsafe-typecast)
